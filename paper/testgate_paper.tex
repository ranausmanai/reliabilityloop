...
framework implementing three complementary reliability strategies: (1) \textit{uncertainty-based
...
(pass@1 improves from 0.54 to 0.72, an 18 percentage point gain). For structured outputs, contract-first generation improves validity
...
We present \textbf{TestGate}, a framework implementing three strategies:
...
\section{The TestGate Framework}

TestGate provides a unified interface for reliability strategies. Given a prompt and model
configuration, it returns a validated output along with metadata about which strategy was applied.
...
\begin{verbatim}
{"columns": [...], "table": "...",
 "where_clauses": [...], "order_by": [...]}
\end{verbatim}
which compiles to: \texttt{SELECT ... FROM ... WHERE ... ORDER BY ...}
...
\textbf{Code generation.} HumanEval~\cite{humaneval} (50 tasks). We measure pass@1 with
...
While 46\% of tasks escalated (vs. 36\% for uncertainty routing), escalation was \textit{always
warranted}---there were no false positives.
...
Code & No & Code-specialized & Uncertainty (cautiously) \\
Code & No & General & Baseline only \\
\midrule
...
We presented TestGate, a framework for improving local LLM reliability through adaptive
...
\item \textbf{Evidence-gating works}: Using test execution as the escalation signal improves
      pass@1 by 18 percentage points (0.54 to 0.72) with zero false positives.
